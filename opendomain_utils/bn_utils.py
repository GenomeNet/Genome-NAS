...
# model, data_loader, mask = model, train_loader_super, supernet_mask
def set_running_statistics(model, data_loader, mask):
    ...
    for i, (input, target) in enumerate(data_loader): # er iteriert jetzt erstmal durch und fügt jedesmal input zusammen bis wir ein riesen input haben mit batch_size 1024 (deswegen bei hidden initialisieren auch batchsize 1024)
        ...
        if len(input_cat) * batchsize < inputsize: # für eine bestimmte anzahl wird input_cat mit input aufgefüllt
        ...
        with torch.no_grad():
            ...
            hidden = forward_model.init_hidden(inputsize) # forward_model ist einfach nur model kopiert, siehe oben bei set_running_statistics
            #print(input.shape)
            #print(hidden.shape)
            
            forward_model(input.to(device), hidden, mask) # supernet_mask
    ...
    for name, m in model.named_modules():
        if name in bn_mean and bn_mean[name].count > 0:
            feature_dim = bn_mean[name].avg.size(0) # einfach nur num_channels=4
