...
## create list with remaining operations ##

# row = np.array([0,0,1,0,0,0,0,0,0])
...
def create_cnn_edge_sampler(cnn_gene):
    ...
    for i in range(4):  
        ...
        for j in range(start, end):
            # j =4
            mask = np.nonzero(cnn_gene[j])[0]#[0] # zweites 0 geht nur, wenn wir nur noch 1 Element haben
            if mask.size == 0:
                cnt += 1
            masks.append(mask)
            #count += 1
        ...
        # add all edges excecpt those which have only one value
        else:
            for e in range(num_edges):
                # e=3
                num_ops = len(masks[e])
...
def create_cnn_supersubnet(supernet_mask, num_disc):
    cnn_gene = copy.deepcopy(supernet_mask[0])
    disc_ops = []

    for i in range(num_disc):
        edge_sampler = create_cnn_edge_sampler(cnn_gene)
        if edge_sampler.size != 0:
            ...
            ops_idxs = np.nonzero(cnn_gene[random_cnn_edge])[0]#[0] # zweites 0 geht nur, wenn wir nur noch 1 Element haben
...
def create_rhn_supersubnet(supernet_mask, num_disc):
    rhn_gene = copy.deepcopy(supernet_mask[1])
    disc_ops = []
    for i in range(num_disc):
