...
parser.add_argument('--num_steps', type=int, default=7, help='number of iterations per epoch')
parser.add_argument('--test_num_steps', type=int, default=7, help='number of iterations per testing epoch')
...
def main():
  ...
  if (args.task == "next_character_prediction"):
      ...
      if (args.model == "DanQ"):
          ...
          model = model.NN_class(num_classes, args.num_motifs, args.batch_size, args.seq_size, args.task).to(device)
  ...
  if (args.task == "TF_bindings"):
      ...
      if (args.model == "DeepSEA"):
          ...
          model = models.DanQ_model.NN_class(num_classes, args.num_motifs, args.batch_size, args.seq_size, args.task).to(device)
          criterion = nn.BCELoss().to(device)
          optimizer = torch.optim.SGD(model.parameters(), lr=args.learning_rate, weight_decay=1e-6, momentum=0.9)
  ...
  for epoch in range(args.epochs):
      ...
      labels, predictions, train_loss = Train(model, train_queue, optimizer, criterion, device, args.num_steps, args.report_freq)
      ...
      if epoch % args.report_freq == 0:
          ...
          labels, predictions, valid_loss = Valid(model, valid_queue, optimizer, criterion, device, args.num_steps, args.report_freq)
  ...
  for epoch in range(args.test_epochs): # 1 epoch has 3000 iterations time 32 batchsize is 96000 samples
      ...
      labels, predictions, test_loss = Valid(model, test_queue, optimizer, criterion, device, args.test_num_steps, args.report_freq)
