...
# -*- coding: utf-8 -*-
"""
Created on Tue Jul 20 18:33:19 2021
...
class CNN_Cell_search(nn.Module):
    ...
    def forward(self, s0, s1, weights):
        ...
        # states = torch.stack([s0,s1], dim=0) # wird zu [4,4,200]
        ...
        for i in range(self._steps):
            # print(i)
            # i=0
            s = sum(self.cell_ops[self.w_pos[i]+j](h, weights[self.w_pos[i]+j]) for j, h in enumerate(states[self.state_idxs[i]])) # vorher node_states und start anstatt offset
            #s = sum(cell_ops[w_pos[i]+j](h, weights[w_pos[i]+j]) for j, h in enumerate(states[state_idxs[i]])) # vorher node_states und start anstatt offset
        ...
        return torch.cat(test[-self._multiplier:], dim=2).squeeze(0)
