...
# -*- coding: utf-8 -*-
"""
Created on Tue Jul 20 20:29:03 2021
...
from darts_tools.comp_aux import get_state_ind, get_w_pos, get_w_range
...
class CNN_Cell_search(nn.Module):
    ...
    def __init__(self, steps, multiplier, C_prev_prev, C_prev, C, reduction, reduction_prev, switches, p, reduction_high):
        ...
        # _steps, switches, reduction, p, C, reduction_high = 4, switches_normal_cnn, False, 0.2, 4, False
        # state_idxs = get_state_ind(_steps, switches)
        
        self.w_pos = get_w_pos(self._steps, switches)
        # w_pos = get_w_pos(_steps, switches)
        
        self.w_pos_range = get_w_range(self._steps, switches)
        # w_pos_range = get_w_range(_steps, switches)
        ...
        # self.cell_ops = nn.Sequential(self.cell_ops)
        # cell_ops = nn.Sequential(cell_ops)
    ...
    def forward(self, s0, s1, weights):
        ...
        states = torch.cat([s0.unsqueeze(0),s1.unsqueeze(0)], dim=0) # wird zu [4,4,200]
        ...
        #a = torch.Tensor([[1,1,1],[1,1,1]]).unsqueeze(0)
        #a1 = torch.Tensor([[2,2,2],[2,2,2]]).unsqueeze(0)
        #a = torch.cat([a,a1], dim=0)

        #b = torch.Tensor([[2,2,2],[2,2,2]]).unsqueeze(0)
        #b1 = torch.Tensor([[3,3,3],[3,3,3]]).unsqueeze(0)
        #b = torch.cat([b,b1], dim=0)
        # b[0]*a[0]
        #torch.sum(a[0:2] * b[[0,1]], dim=0)
        #torch.sum(a[0:2] * b[0:2], dim=
        
        #states = [1,2,3]
        #torch.sum(a[0:2] * states[0:2])
        #s0 = torch.Tensor([1])
        #s1 = torch.Tensor([2])
        #states = torch.cat([s0,s1], dim=0)
        #a[0]*states[1]
        ...
        #alpha_list = alpha_list.append(torch.nn.Parameter(s0))
        ...
        for i in range(self._steps):
            ...
            s = torch.sum(self.cell_ops[self.w_pos_range[i][0]:self.w_pos_range[i][1]](states[self.state_idxs[i]], weights[self.w_pos_range[i][0]:self.w_pos_range[i][1]]), dim=0) # vorher node_states und start anstatt offset
            #s = torch.sum(cell_ops[w_pos_range[i][0]:w_pos_range[i][1]](states[state_idxs[i]], weights[w_pos_range[i][0]:w_pos_range[i][1]])) # vorher node_states und start anstatt offset
            
            #cell_ops[0](states[0], weights[0]) + cell_ops[1](states[1], weights[1])
            
            #for j, cell in enumerate(cell_ops):
            #    torch.sum(cell(states[0:2],weights[0:2])
            #    cell()
            #torch.sum(cell(states[0:2], weights[0:2]) , dim=0)
            ...
            # states.append(s..unsqueeze(0))states[state_idxs[i]]
            states = torch.cat([states, s.unsqueeze(0)], dim=0)
            

        #return torch.cat(states[-self._multiplier:], dim=1)
        test = torch.split(states, 1)
        return torch.cat(test[-self._multiplier:], dim=2)
    
    #states1 = torch.cat(states, dim=1)
    
    #test = torch.split(states, 1)
    #states2 = torch.cat(test, dim=2)
